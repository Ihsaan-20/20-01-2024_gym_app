...
@section('app')
    ...
    //  @dd($data->id);
    // @dd($data->program);
      $programData = json_decode($data->program);  

    // @dd($programData->workouts)

    @endphp
    <form action="{{ route('form.update') }}" method="POST" enctype="multipart/form-data" class="px-3">
            @csrf
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="pro-name" class="form-label">Program Name</label>
                    <input type="hidden" class="form-control" name="id"  value="{{ $data->id }}" />
                    <input type="text" class="form-control" name="program_name"  value="{{ old('program_name', $programData->program_name) }}" />
                </div>
                
                <div class="col-md-6 mb-3">
                    <label for="file" class="form-label">Introductory Video</label>
                    <input type="hidden" class="form-control" value="{{ is_string($programData->program_intro_video) ? $programData->program_intro_video : '' }}" name="program_intro_video"  />
                    <input type="file" class="form-control" name="program_intro_video"  />
                    <span class="text-muted">{{ is_string($programData->program_intro_video) ? $programData->program_intro_video : 'not uploaded!' }}</span>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="form-floating">
                        <textarea class="form-control" placeholder="Leave a comment here" name="program_text_bio"
                            id="floatingTextarea">{{ old('program_name', $programData->program_text_bio)  }}</textarea>
                ...
                <div class="mb-3 col-md-6">
                    ...
                    <select multiple="multiple" name="program_equi_needed" class="form-control equipmentNeeded">
                        <option value="body" {{ $programData->program_equi_needed === 'body' ? 'selected' : '' }}>Body</option>
                        <option value="legs" {{ $programData->program_equi_needed === 'legs' ? 'selected' : '' }}>Legs</option>
                        <option value="back" {{ $programData->program_equi_needed === 'back' ? 'selected' : '' }}>Back</option>
                        <option value="shoulder" {{ $programData->program_equi_needed === 'shoulder' ? 'selected' : '' }}>Shoulder</option>
                    </select>
                </div>
                <div class="mb-3 col-md-6">
                    ...
                    <select multiple="multiple" name="program_training_type" class="form-control trainingType">
                        <option value="body" {{ $programData->program_training_type === 'body' ? 'selected' : '' }}>Body</option>
                        <option value="legs" {{ $programData->program_training_type === 'legs' ? 'selected' : '' }}>Legs</option>
                        <option value="back" {{ $programData->program_training_type === 'back' ? 'selected' : '' }}>Back</option>
                        <option value="shoulder" {{ $programData->program_training_type === 'shoulder' ? 'selected' : '' }}>Shoulder</option>
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" name="coach_id" aria-label="Default select example">
                        ...
                        <option value="1" {{ $programData->coach_id === 1 ? 'selected' : '' }}>Harry</option>
                        <option value="2" {{ $programData->coach_id === 2 ? 'selected' : '' }}>Peter</option>
                        <option value="3" {{ $programData->coach_id === 3 ? 'selected' : '' }}>Jason</option>
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="file" class="form-label">Profile Picture</label>
                    <input type="hidden" class="form-control" value="{{ $programData->program_profile??'' }}" name="program_profile" id="file" />
                    <input type="file" class="form-control"  name="program_profile" id="file" />
                    <span class="text-muted">{{ $programData->program_profile?? 'not uploaded!' }}</span>
                </div>
                <div class="mb-3 col-md-6">
                    <label for="timeInput" class="form-label">Duration</label>
                    <input type="time" name="program_duration" value="{{  $programData->program_duration }}" class="form-control" />
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" name="program_level" aria-label="Default select example">
                        ...
                        <option value="beginner" {{ $programData->program_level === 'beginner' ? 'selected' : '' }}>Beginner</option>
                        <option value="intermediate" {{ $programData->program_level === 'intermediate' ? 'selected' : '' }}>Intermediate</option>
                        <option value="advanced" {{ $programData->program_level === 'advanced' ? 'selected' : '' }}>Advanced</option>
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="file" class="form-label">Price (USD)</label>
                    <input type="tel" class="form-control" id="usdPrice" name="program_price" value="{{ $programData->program_price }}" placeholder="1"
            ...
            <div>
              @php
                  $workoutCount = 0;
              @endphp
             @foreach ($programData->workouts as $key => $workout)
             @php
                  $workoutCount++;
             @endphp
              @include('form._partial_workout_edit', ['workout' => $workout, 'workoutCount' => $workoutCount])
             @endforeach
            </div>
            {{-- edit portion --}}
